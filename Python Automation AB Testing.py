...
# ^ this is for when you want to study overlaps: you can export data from MySQL workbench
# you can also set your export file path as your current directory
...
def z_test_calculator(df,denominator,numerator):
    #df: df_Result - dataframe containing the result (absolute counts) - this is the df_result from previous step
    #denominator: str (column name)
    #numerator:str
    
    
    #get the data, and transform
    #https://www.w3resource.com/pandas/dataframe/dataframe-loc.php#:~:text=DataFrame%20%2D%20loc%20property,used%20with%20a%20boolean%20array.
...
KPIs=[('SessionID','ReachedCheckout') #tuple
      ,('SessionID','Converted')]
...
for index in df_Result.index:
    j=0 #initializize counter
    if index!=0:
        df_each_group=df_Result.loc[[0,index],]
        df_each_group.index=[1,0]
        
        # this is a dataframe, otherwise the df_fianl.loc lines won't work
...
# ## Week 6: Granularity, Cuts & Continunous Variables
# We are going to perform both the Mann-Whitney U Test and a Z-score calculation
...
for index in df_Result.index:
    ...
    if index!=0:
        ...
        df_final=pd.DataFrame() # your original df_final will disappear
...
cut='VisitorTypeID' # define a dimension to cut the data by
...
# Mann-Whitney test only reads arrays
...
Control_Rev # data type is float64
...
# note: we did not abbreviate the name of the package in the beginning
...
# re-declare df_Result, so that the results are not cut by category
df_Result=pd.DataFrame(df_test2.groupby('TestGroup')['SessionID'].count())
metrics=['Bounced','SawProduct','AddedToCart','ReachedCheckout','Converted']

gc.collect()

for metric in metrics:
    a=df_test2[df_test2[metric]==1].groupby('TestGroup')['SessionID'].count()
    a.name=metric
    df_Result=df_Result.join(a)
...
# transform the arrays to df columns to get sum total 
df_Result.loc[0,'Rev']=sum(Control_Rev)
df_Result.loc[1,'Rev']=sum(Var1_Rev)
# add sum squared total
df_Result.loc[0,'Rev_sq']=sum(Control_Rev**2) # x1^2+x2^2+...+xn^2
df_Result.loc[1,'Rev_sq']=sum(Var1_Rev**2)
...
# if you want to surpress the scientific notation
# pd.options.display.float_format='{:.2f}'.format
df_Result
# if you don't like it you can also reset the option 
...
def z_test_calculator_continuous(df,denominator,numerator,numerator_sq):
    ...
    #Rate: actually not a rate, but an average
    control_rate=control_numerator/control_denominator
    var_rate=var_numerator/var_denominator
    
    #Variance
    control_var=df.loc[0,numerator_sq]/control_denominator-control_rate**2
    var_var=df.loc[1,numerator_sq]/var_denominator-var_rate**2
...
# declare a new dictionary for the revenue metrics
dic_rev_final={}
for index in df_Result.index:
    j=0
    if index!=0:
...
# you've already imported these
# import numpy as np
# import matplotlib.pyplot as plt
# Ctrl+/ (windows) or Cmnd+/ (mac) for block comment

# transform control data, and normalize skewed & large values
x=df_rev.loc[df_rev['ControlGroup']==1,'TotalRev'].apply(lambda x: math.sqrt(abs(x)))
n_bins=50
fig,ax=plt.subplots(figsize=(8,4))
# fig: your figure/image object
# ax: your axes object - store and manipulate
# https://stackoverflow.com/questions/34162443/why-do-many-examples-use-fig-ax-plt-subplots-in-matplotlib-pyplot-python
n,bins,patches=ax.hist(x, n_bins,density=True,histtype='step',cumulative=True,label='Cumulative') # this is a CDF plot
...
# change the input of z-score calculator to exclude 95th percentile outliers
...
df_test_data=df_test2.copy() # re-declare or rename
...
# same values, but we turn series into dataframes for further processing
